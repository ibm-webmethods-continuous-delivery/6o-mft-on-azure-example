#!/usr/bin/env bash
#
# Deploy ActiveTransfer Gateway 1 using Azure VM Run Command
...
#   export VM_NAME=<gateway1-vm-name>
...
if [[ -z "${VM_NAME}" ]]; then
    ...
    echo "Usage: export VM_NAME=<gateway1-vm-name>"
    exit 1
fi
...
mft.server.id=Gateway1-Azure
mft.server.name=Gateway1-Azure
...
docker ps | grep at-gateway1 || true

echo "==> Checking container logs (last 20 lines)..."
docker logs at-gateway1 --tail 20 || true

echo ""
echo "=========================================="
echo "Gateway 1 deployment completed!"
echo "=========================================="
echo ""
...
echo ""
echo "To view logs:"
echo "  docker logs at-gateway1 -f"
echo ""
echo "To restart:"
...
echo "   az vm run-command invoke -g ${RESOURCE_GROUP} -n ${VM_NAME} --command-id RunShellScript --scripts 'docker logs at-gateway1 --tail 50'"
echo ""
echo "3. Test connectivity from AKS (after deploying Gateway 2):"
echo "   kubectl run test-gateway --image=busybox --rm -it --restart=Never -n mft -- nc -zv 10.1.0.4 8500"
echo ""
